import { ArchiveBook, ArrowDown2, Brush2, Trash } from "iconsax-react";
import { showFormattedDate } from "../utils/data";

function MyCatatan(props) {
  //
      ...
      listCatatans.map((data) => (
        <div key={data.id} className="p-4 bg-amber-300 rounded-lg mb-4">
          ...
          <div className="mt-1">
            ...
            >
              <ArchiveBook className="text-amber-800 hover:text-amber-600 transition-colors" />
            </button>
            <button
              title="Ubah"
              className="mr-4"
  ...
  return (
    <div id="my-catatan" className="border-t border-amber-300 p-4">
      <h2 className="text-xl font-semibold mb-4">Catatan Ku</h2>
      <div className="list">{listCatatansElm}</div>
    </div>
  );
}

export default MyCatatan;
